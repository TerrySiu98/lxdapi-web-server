...
configure_lxdapi() {
    ...
    reading "请输入日志自动清理天数 [7]：" auto_cleanup_days
...
EOF
    ...
    sed -i "s|__API_KEY__|$api_key|g" "$config_file"
...
main() {
    ...
    sys_arch=$(uname -m)
    case $sys_arch in
        x86_64) exec_bin="/opt/lxdapi/lxdapi-amd64" ;;
        aarch64|arm64) exec_bin="/opt/lxdapi/lxdapi-arm64" ;;
    esac
    
    machine_id=$($exec_bin --machine-id 2>/dev/null)
    info "授权站: vpsm.link"
    info "机器码: $machine_id"
    echo
    reading "请输入授权密钥：" license_key
    if [ -n "$license_key" ]; then
        echo "$license_key" > /opt/lxdapi/license.key
        ok "授权文件已写入"
        setup_lxdapi_service
    else
        warn "未输入授权密钥，服务未启动"
